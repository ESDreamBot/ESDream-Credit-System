...
client.on('message',async message => {
  ...
  const daily = Math.floor(Math.random() * 100000) + 100000;
    ...
    message.channel.send(`**:heavy_dollar_sign:| \`${number}\`, أكتب الرقم للأستمرار :pencil: **`).then(m => {
      message.channel.awaitMessages(m => m.author.id === message.author.id, {max: 1, time: 10000}).then(c => {
        if(c.first().content === number) {
          m.delete();
          message.channel.send(`**:moneybag: | ${message.author.username}, قام بتحويل \`${balance}\` لـ ${mention} :money_mouth:  **`);
...
client.user.setGame(`=credit | =daily`,"https://www.twitch.tv/idk")
